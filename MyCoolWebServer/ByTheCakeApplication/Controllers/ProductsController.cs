namespace MyCoolWebServer.ByTheCakeApplication.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        public IHttpResponse Add(AddProductViewModel model)
        {
            ...
            {
                this.ApplyError("Product Information is not valid.");
        ...
        public IHttpResponse Search(IHttpRequest req)
        {
            ...
            else
            {
                var allProducts = result
                    .Select(c => $@"<div><a href=""/cakes/{c.Id}"">{c.Name}</a> - ${c.Price} <a href=""/shopping/add/{c.Id}?searchTerm={searchTerm}"">Order</a></div>");
